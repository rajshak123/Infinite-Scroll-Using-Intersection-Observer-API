...
class List extends PureComponent {
  static propTypes = {
    ...
    items(props, propName) {
      ...
      if (
        object != null &&
        !(
          Array.isArray(object) || typeof object[Symbol.iterator] === 'function'
        )
      ) {
        return new Error(
  ...
  static getDerivedStateFromProps({ pageSize, ...props }, prevState) {
    const itemCount = getItemCount(props);

    let newSize;
    if (prevState.size === 0) {
      newSize = pageSize;
    } else if (prevState.itemCount < itemCount) {
  ...
  setRef = callback => {
    ...
    this.setRootNode = node => {
      if (node !== prevRootNode) {
        ...
        callback(
          ['auto', 'scroll', 'overlay'].indexOf(overflow) !== -1 ? node : null,
        );
      }
    };
  };
